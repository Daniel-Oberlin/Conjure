...
// ~7 ms, spiking to ~11 ms, on the same frame as the render). Step 1 hosts SpaceSnap.register; matchRef /
// snap / anchor-solve can migrate behind this SAME message boundary as scenes grow — never touching the
// frame budget again.
//
// Module worker, because: three 0.163 is ESM-only (no UMD to importScripts), and space-snap.js is UMD —
// imported for side effect, its wrapper takes the `else` branch (no CommonJS `module` here) and assigns
// `self.SpaceSnap`. All SpaceSnap math takes THREE as its first argument, so this worker's own standalone
...
//
// Imports are DYNAMIC + version-tagged: the main thread spawns us as `space-worker.js?v=<mtime>` and we
// forward that `?v=` onto our own imports, so a change to space-snap.js/three busts the worker's cached copy
...
await import("./space-snap.js" + _q);                       // UMD side effect → self.SpaceSnap
var SpaceSnap = self.SpaceSnap;
...
self.onmessage = function (e) {
  ...
  var r = SpaceSnap.register(THREE, cur, ref, m.opts);
