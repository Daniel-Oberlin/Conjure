...
DIRECTOR_PROMPT = (
    ...
    "texture_surface(target, image_id) where target is a semantic label ('floor'/'ceiling'/'wall') or "
    "'all'; pass repeat=N with a seamless/tileable image to tile it (grass, brick). To color a surface "
    "or make it see-through, use style_surface(target, color, opacity) ('glass walls' = low opacity). "
    "Use show_annotations(on) to label each surface with its name + id when the user wants to identify "
    "or reference surfaces. Don't pick an image generator unless the user asked for a specific one — "
